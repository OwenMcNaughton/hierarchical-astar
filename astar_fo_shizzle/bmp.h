...
struct BMP {
  ...
  bool is_pixel_black(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
  ...
  void print_pixel(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    auto xx = data[channels * (y * bmp_info_header.width + x) + 2];
    std::cout << int(data[channels * (y * bmp_info_header.width + x) + 2])
              << " ";
  }

  bool is_pixel_green(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
  ...
  bool is_pixel_cyan(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 255 &&
           data[channels * (y * bmp_info_header.width + x) + 1] == 255 &&
           data[channels * (y * bmp_info_header.width + x) + 2] == 0;
  }

  bool is_pixel_red(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
           data[channels * (y * bmp_info_header.width + x) + 1] == 0 &&
  ...
  bool is_pixel_blue(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 255 &&
           data[channels * (y * bmp_info_header.width + x) + 1] == 0 &&
           data[channels * (y * bmp_info_header.width + x) + 2] == 0;
  ...
  bool is_pixel_yellow(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
           data[channels * (y * bmp_info_header.width + x) + 1] == 255 &&
           data[channels * (y * bmp_info_header.width + x) + 2] == 255;
  ...
  bool is_pixel_building(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return is_residential(x, y) || is_commercial(x, y);
  }

  bool is_residential(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return (data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
            data[channels * (y * bmp_info_header.width + x) + 1] == 0 &&
            data[channels * (y * bmp_info_header.width + x) + 2] == 100);
  }

  bool is_commercial(uint32_t x, uint32_t y) {
    uint32_t channels = bmp_info_header.bit_count / 8;
    return (data[channels * (y * bmp_info_header.width + x) + 0] == 0 &&
            data[channels * (y * bmp_info_header.width + x) + 1] == 0 &&
            data[channels * (y * bmp_info_header.width + x) + 2] == 200);
  }
